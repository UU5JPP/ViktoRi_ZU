...
*/
  ...
  void start(uint8_t i) {
    ...
    writeRegister(INA226_REG_ALERTLIMIT, (uint16_t)trunc(((float)EEPROM.read(8 + 3) * 3276.8f / _i_max)));  // Alert - ограничение по максимальному току   // 0,081/0,0000025f   ограничение по напряжению на шунте 32400 * 0.0025 = 81мВ
    ...
    switch (i) {
      case 1:
       ...
       setAveraging(INA226_AVG_X1);                    // усреднение, по умолчанию усреднения нет(1, 4, 16, 64, 128, 256, 512, 1024).
        _prd = 920;                                      //670 780 / 846 Гц
        break;
      case 2:
