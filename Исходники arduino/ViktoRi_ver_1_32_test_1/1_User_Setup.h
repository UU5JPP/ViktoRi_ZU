...
#define SENSTEMP1 1  // датчик температуры транзистора Q1(подкл. к пин 3): 1 - DS18B20, 2 - NTC, 0 - не используется.  Схема 1.6.3
...
#define SENSTEMP2 2  // датчик температуры аккумулятора(подкл. к пин 6): 1 - DS18B20(для LGT8F328p) , 2 - NTC, 0 - не используется.  Схема 1.6.3
// параметры для  датчика температуры NTC
...
// управление кулером при разряде
#define KULDISCHAR 1  // 0 - управление кулером по датчику темпрературы SENSTEMP1. 1 - принудительное включение вентилятора при разряде
...
// уменьшение тока заряда при превышении температуры силового транзистора. Работает совместно с SENSTEMP1
...
// Логгер. Отправка в сетевой порт. Занимает много памяти. Если хочешь ее использовать отключи другие функции. 
#define LOGGER 0    // функция отправки значений в сетевой порт
                    //  1 - набор логов: напряжение, ток, А/ч, (напряжение БП), (темп Q1), (темп Акб) - в скобках вывод при наличии датчиков иначе выводятся нули
                    //  2 - вывод в порт структуры данных c CRC: смотри Classes.h строка 414 (напряжение, ток, А/ч, напряжение БП, темп Q1, темп Акб)
                    //  3 - вывод в порт логов для Serial Print Plotter
                    //  0 - не используется.
#define LOGGTIME 1  // период, в секундах, 1 - 255. С этим периодом будут отправляться данные в сетевой порт.
#define SERIAL_SPEED 9600  // 9600 - оптимально
...
#define PINTERM1 A3  // вход датчика температуры транзистора - DS18B20 или NTC
#define PINTERM2 A6  // вход датчика температуры акб - DS18B20 или NTC
