...
#define INTERFACE 0  // выбор языка интерфейса: 0 - русский(!для дисплеев с поддержкой русского языка!), 1 - английский, 2 - транслит
// параметры дисплея
#define ADDRDISP 0x27  // адрес дисплея 0x27 или 0x3F
#define DISPLAYx 20    // количество столбцов дисплея 16 или 20
#define DISPLAYy 4     // количество строк дисплея     2 или 4
...
#define POWPIN 2   // 0 - не используется, 1 - реле 220 Вольт, 2 - индикатор включения БП в сеть(не работает без #define VOLTIN 1)
...
// сопротивление шунта
#define SHUNT 0.01700                       // сопротивление шунта, Ом
...
#define VREF 5012     // точное напряжение на пине 5V (в мВ). Замерь напряжение и впиши сюда значение. Влияет на замер напряжения от БП. 4292  4304
...
#define NTCRS2 10000  // Ом - сопротивление подтягивающего резистора (тем точнее тем лучше)
...
// время дисплея
#define TIME_LIGHT 0  // время подсветки дисплея в минутах (от 0 до 255 минут). При 0 подсветка отключаться не будет. 
#define TIME_DISP 10  // время возврата на 1 экран в секундах
// профили
#define PROFIL 5  // количество профилей пользователя 0 - 9.
...
//график напряжения и тока при заряде за последний час.
#if defined(__LGT8FX8P__)
#define GRAFIK 0  // 1 - вкл, 0 - откл.  Данные фиксируются каждые 5 минут. В LGT8 не влезает.
#else
#define GRAFIK 1  // 1 - вкл, 0 - откл.  Данные фиксируются каждые 5 минут. В LGT8 не влезает.
#endif
...
#define LOGGER 0    // функция отправки значений в сетевой порт (скорость 115200).  Занимает много памяти. Если хочешь ее использовать отключи другие функции. 
                    //  1 - набор логов: время, напряжение, ток, А/ч, Вт/ч, темп Акб, темп Q1, напряжение БП. 
                    //  2 - набор логов: время, напряжение, ток. 
                    //  0 - не используется.
...
#define PINTERM2 A6  // вход датчика температуры NTC 
