...
public:
    ...
    float getTemp() {
        return computeTemp(analogRead_my(_pin), _res);
    }

    int8_t getTempInt() {
        return (int8_t)computeTemp(analogRead_my(_pin), _res);
    }
    
    // прочитать усреднённую температуру с пина, можно указать к-во усреднений
    float getTempAverage(uint8_t samples = 8) {
        uint16_t aver = 0;
        for (uint8_t i = 0; i < samples; i++) aver += analogRead_my(_pin);
